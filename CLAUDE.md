...
4. **RegionDetector** (`pipeline/detector.py`) — YOLOv8n; `crop_all()` first checks `_has_trained_boxes()` — does the loaded model have any class matching `{predicted_key}_*` (prefix, e.g. `back_label_lot`/`_name`/`_size`) or an exact match (e.g. `capsule_box`, which has no underscore suffix)? Only if so does it run YOLO inference and keep every matching detected box, sorted top→bottom (single-region then stacks them). A class with no matching trained boxes at all (e.g. `30_sachet`) skips YOLO inference entirely and goes straight to the heuristic full-image fallback — it never accepts a box trained for a different packaging class. It does NOT read `detector_yolo_prefixes` — that field is consumed only by edit-draft prelabel (`active_learning`). Also falls back to heuristics if YOLO returns zero boxes for a class that does have trained boxes.
