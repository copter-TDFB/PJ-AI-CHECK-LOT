...
- The working tree often holds several in-progress features at once. Before `git add <file>`, run `git diff <file>` — staging a whole file can bundle unrelated pre-existing changes into your commit (`git add -p` is unavailable here; stage deliberately).
